  ...
  const fetchData = useCallback(async () => {
    try {
      ...
      const res = await axios.get(
        "https://radar-backend.onrender.com/api/blog/own",
        {
          headers: {
  ...
  const handleConfirmDelete = async () => {
    try {
      const isDelete = await axios.delete(
        `https://radar-backend.onrender.com/api/blog/delete/${blogIdToDelete}`,
        {
          headers: {
            Authorization: `Bearer ${token}`,
            "Content-Type": "application/json",
      ...
      if (isDelete) {
        fetchData();
        setDeleteDialogOpen(false);
        toast.success(isDelete.data.msg);
      }
    } catch (error) {
      console.error("Response Error:", error.response.data);
      toast.error(
        error.response.data.msg || "An error occurred while deleting the blog."
      );
      setDeleteDialogOpen(false);
    }
  };
                ...
                <Card sx={{ width: "100%", margin: "auto" }}>
                  <CardHeader
                    ...
                    action={
                      <IconButton
                        ...
                        onClick={() => navigate(`/addblog/${item._id}`)}
                      >
                        <EditIcon />
                      </IconButton>
                    }
                  ...
                  <CardContent sx={{ flexGrow: 1 }}>
                    ...
                    <Typography variant="body2" color="text.secondary">
                      Blog Category Description: {item.category_desc}
                    </Typography>
                  </CardContent>
